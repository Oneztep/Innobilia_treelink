...
export default function ShareModal({ property, isOpen, onClose, onConfirmShare, dialogRef }: ShareModalProps) {
  ...
  return (
    <dialog className="fixed inset-0 z-50 flex items-center justify-center p-4" ref={dialogRef} aria-modal="true" aria-label="Compartir propiedad">
      <div className={`fixed inset-0 bg-slate-900/60 backdrop-blur-md-custom ${animClass.overlay}`} aria-label='Close-overlay' onClick={close} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') close(); }} role="button" tabIndex={0} />
